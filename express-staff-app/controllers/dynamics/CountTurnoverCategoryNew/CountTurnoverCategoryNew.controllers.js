...
function transformData(input) {
  ...
  input.forEach(item => {
    const { label, category_name, turnover_count } = item;
    ...
    if (dataByCategory[category_name]) {
      dataByCategory[category_name].push(turnover_count);
    } else {
      // Если категория еще не существует, создаем новый объект данных для нее
      dataByCategory[category_name] = [turnover_count];
    }
  });
...
exports.getCountTurnoverCategoryNew= async (req, res) => {
  try {
    let inputArray = await db.sequelize.query(SQL.CountTurnoverCategoryNew, {
...
exports.getCountTurnoverCategoryNewOrganization = async (req, res) => {
  try {
    const organizationIds = req.body.data.map(item => item.id); 
    let inputArray = await db.sequelize.query(SQL.CountTurnoverCategoryNewOrganization, {
      replacements: {
