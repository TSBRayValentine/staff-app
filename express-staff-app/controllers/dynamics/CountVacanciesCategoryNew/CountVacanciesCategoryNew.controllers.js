...
function transformData(input) {
  ...
  input.forEach(item => {
    const { label, category_name, vacancies_count } = item;
    ...
    if (dataByCategory[category_name]) {
      dataByCategory[category_name].push(vacancies_count);
    } else {
      // Если категория еще не существует, создаем новый объект данных для нее
      dataByCategory[category_name] = [vacancies_count];
    }
  });
...
exports.getCountVacanciesCategoryNew= async (req, res) => {
  try {
    let inputArray = await db.sequelize.query(SQL.CountVacanciesCategoryNew, {
...
exports.getCountVacanciesCategoryNewOrganization = async (req, res) => {
  try {
    const organizationIds = req.body.data.map(item => item.id); 
    let inputArray = await db.sequelize.query(SQL.CountVacanciesCategoryNewOrganization, {
      replacements: {
