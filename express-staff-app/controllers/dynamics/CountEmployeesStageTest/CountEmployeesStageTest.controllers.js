...
exports.getCountEmployeesStageTest= async (req, res) => {
  try {
    const groupcategoryIds = req.body.data.groupcategory.map(item => item.id);
    let inputArray = await db.sequelize.query(SQL.CountEmployeesStageTest, {
...
exports.getCountEmployeesStageTestOrganization = async (req, res) => {
  try {
    const groupcategoryIds = req.body.data.groupcategory.map(item => item.id);
    const organizationIds = req.body.data.organization.map(item => item.id);
  
    let inputArray = await db.sequelize.query(SQL.CountEmployeesStageTestOrganization, {
      replacements: {
