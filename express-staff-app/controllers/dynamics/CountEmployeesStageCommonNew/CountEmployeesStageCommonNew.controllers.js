...
exports.getCountEmployeesStageCommonNew= async (req, res) => {
  try {
    let inputArray = await db.sequelize.query(SQL.CountEmployeesStageCommonNew, {
...
exports.getCountEmployeesStageCommonNewOrganization = async (req, res) => {
  try {
    const organizationIds = req.body.data.map(item => item.id); 
    let inputArray = await db.sequelize.query(SQL.CountEmployeesStageCommonNewOrganization, {
      replacements: {
