...
function transformData(input) {
  ...
  input.forEach(item => {
    const { label, category_name, count_doctor } = item;
    ...
    if (dataByCategory[category_name]) {
      dataByCategory[category_name].push(count_doctor);
    } else {
      // Если категория еще не существует, создаем новый объект данных для нее
      dataByCategory[category_name] = [count_doctor];
    }
  });
...
exports.getCountDoctorCategoryNew= async (req, res) => {
  try {
    let inputArray = await db.sequelize.query(SQL.CountDoctorCategoryNew, {
...
exports.getCountDoctorCategoryNewOrganization = async (req, res) => {
  try {
    const organizationIds = req.body.data.map(item => item.id); 
    let inputArray = await db.sequelize.query(SQL.CountDoctorCategoryNewOrganization, {
      replacements: {
