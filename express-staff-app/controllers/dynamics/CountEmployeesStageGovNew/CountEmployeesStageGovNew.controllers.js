...
exports.getCountEmployeesStageGovNew= async (req, res) => {
  try {
    let inputArray = await db.sequelize.query(SQL.CountEmployeesStageGovNew, {
...
exports.getCountEmployeesStageGovNewOrganization = async (req, res) => {
  try {
    const organizationIds = req.body.data.map(item => item.id); 
    let inputArray = await db.sequelize.query(SQL.CountEmployeesStageGovNewOrganization, {
      replacements: {
