...
function transformData(input) {
  ...
  input.forEach(item => {
    const { label, category_name, count_with_honors } = item;
    ...
    if (dataByCategory[category_name]) {
      dataByCategory[category_name].push(count_with_honors);
    } else {
      // Если категория еще не существует, создаем новый объект данных для нее
      dataByCategory[category_name] = [count_with_honors];
    }
  });
...
exports.getCountWithHonorsCategoryNew= async (req, res) => {
  try {
    let inputArray = await db.sequelize.query(SQL.CountWithHonorsCategoryNew, {
...
exports.getCountWithHonorsCategoryNewOrganization = async (req, res) => {
  try {
    const organizationIds = req.body.data.map(item => item.id); 
    let inputArray = await db.sequelize.query(SQL.CountWithHonorsCategoryNewOrganization, {
      replacements: {
