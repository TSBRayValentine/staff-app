...
const getCountEmployees = async () => { 
  ...
  if (props.organization.length != 0) {
    API = 'dynamics/getCountVacanciesCategoryNewOrganization'      

    result = await axios.post(K.API_URL + API, { data: props.organization });

    options.value = {
      ...options.value, ...{
        title: { text: `| Кол-во свободных вакансий для выбранных организаций| (${props.organization.length}) |`, }
      }
    }
  }
  else {
    API = 'dynamics/getCountVacanciesCategoryNew'
    result = await axios.get(K.API_URL + API);

    options.value = {
      ...options.value, ...{
        title: { text: `| Кол-во свободных вакансий для всех организаций |`, }
      }
    }
  }
