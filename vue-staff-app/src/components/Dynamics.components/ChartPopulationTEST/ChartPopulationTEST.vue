...
const getCountEmployees = async () => {
  ...
  if (props.organization.length != 0) {
    API = "dynamics/getPopulationTestOrganization";

    result = await axios.post(K.API_URL + API, {
      data: {
    ...
    options.value = {
      ...
      ...{
        title: {
          text: `| Распределение по полу для выбранных организаций | (${props.organization.length}) |`,
        },
      },
    };
  } else {
    API = "dynamics/getPopulationTest";
    result = await axios.post(K.API_URL + API, {
      data: { groupcategory: props.GroupCategory },
    });

    options.value = {
      ...options.value,
      ...{
        title: { text: `| Распределение по полу для всех организаций |` },
      },
    };
  }
