...
const getCountEmployees = async () => { 
  ...
  if (props.organization.length != 0) {
    API = 'dynamics/getCountCandidateCategoryNewOrganization'      

    result = await axios.post(K.API_URL + API, { data: props.organization });

    options.value = {
      ...options.value, ...{
        title: { text: `| Кол-во сотрудников с кандидатской для выбранных организаций| (${props.organization.length}) |`, }
      }
    }
  }
  else {
    API = 'dynamics/getCountCandidateCategoryNew'
    result = await axios.get(K.API_URL + API);

    options.value = {
      ...options.value, ...{
        title: { text: `| Кол-во сотрудников с кандидатской для всех организаций |`, }
      }
    }
  }
