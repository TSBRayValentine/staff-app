...
const getCountEmployees = async () => { 
  ...
  if (props.organization.length != 0) {
    API = 'dynamics/getCountEmployeesNewOrganization'      

    result = await axios.post(K.API_URL + API, { data: props.organization });

    options.value = {
      ...options.value, ...{
        title: { text: `| Кол-во сотрудников для выбранных организаций | (${props.organization.length}) |`, }
      }
    }
  }
  else {
    API = 'dynamics/getCountEmployeesNew'
    result = await axios.get(K.API_URL + API);

    options.value = {
      ...options.value, ...{
        title: { text: `| Общее кол-во для всех организаций |`, }
      }
    }
  }
