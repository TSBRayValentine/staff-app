<template>
  ...
  const getCountEmployees = async () => { 
    ...
    if (props.organization.length != 0) {
      API = 'dynamics/getCountEmployeesStageTestOrganization'      
  
      result = await axios.post(K.API_URL + API, { data: {organization: props.organization, groupcategory: props.GroupCategory} });
  
      options.value = {
        ...options.value, ...{
          title: { text: `| Стаж в организаци для выбранных организаций | (${props.organization.length}) |`, }
        }
      }
    }
    else {
      API = 'dynamics/getCountEmployeesStageTest'
      result = await axios.post(K.API_URL + API, { data: { groupcategory: props.GroupCategory} });
  
      options.value = {
        ...options.value, ...{
          title: { text: `| Стаж в организаци для всех организаций |`, }
        }
      }
    }
