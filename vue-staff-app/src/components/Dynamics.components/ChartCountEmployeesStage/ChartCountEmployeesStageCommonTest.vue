<template>
  ...
  const getCountEmployees = async () => { 
    ...
    if (props.organization.length != 0) {
      API = 'dynamics/getCountEmployeesStageCommonTestOrganization'      
  
      result = await axios.post(K.API_URL + API, { data: {organization: props.organization, groupcategory: props.GroupCategory} });
  
      options.value = {
        ...options.value, ...{
          title: { text: `| Общий стаж для выбранных организаций | (${props.organization.length}) |`, }
        }
      }
    }
    else {
      API = 'dynamics/getCountEmployeesStageCommonTest'
      result = await axios.post(K.API_URL + API, { data: { groupcategory: props.GroupCategory} });
  
      options.value = {
        ...options.value, ...{
          title: { text: `| Общий стаж для всех организаций |`, }
        }
      }
    }
