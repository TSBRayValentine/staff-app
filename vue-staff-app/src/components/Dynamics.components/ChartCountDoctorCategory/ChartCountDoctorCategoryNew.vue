...
const getCountEmployees = async () => { 
  ...
  if (props.organization.length != 0) {
    API = 'dynamics/getCountDoctorCategoryNewOrganization'      

    result = await axios.post(K.API_URL + API, { data: props.organization });

    options.value = {
      ...options.value, ...{
        title: { text: `| Кол-во сотрудников с докторской для выбранных организаций| (${props.organization.length}) |`, }
      }
    }
  }
  else {
    API = 'dynamics/getCountDoctorCategoryNew'
    result = await axios.get(K.API_URL + API);

    options.value = {
      ...options.value, ...{
        title: { text: `| Кол-во сотрудников с докторской для всех организаций |`, }
      }
    }
  }
