<template>
  ...
  const getCountEmployees = async () => { 
    ...
    if (props.organization.length != 0) {
      API = 'dynamics/getCountEmployeesAgesTestOrganization'      
  
      result = await axios.post(K.API_URL + API, { data: {organization: props.organization, groupcategory: props.GroupCategory} });
  
      options.value = {
        ...options.value, ...{
          title: { text: `| Распределение по возрасту для выбранных организаций | (${props.organization.length}) |`, }
        }
      }
    }
    else {
      API = 'dynamics/getCountEmployeesAgesTest'
      result = await axios.post(K.API_URL + API, { data: { groupcategory: props.GroupCategory} });
  
      options.value = {
        ...options.value, ...{
          title: { text: `| Распределение по возрасту для всех организаций |`, }
        }
      }
    }
